          ...
          {cards.map((card) => {
            ...
            return (
              <button
                ...
                className="group text-left cursor-pointer relative overflow-hidden rounded-3xl p-7 transition-all duration-500"
                style={{
                  background: 'linear-gradient(180deg, rgba(28,24,42,0.85) 0%, rgba(14,12,22,0.92) 100%)',
                  border: '1px solid rgba(255,255,255,0.06)',
                  backdropFilter: 'blur(20px)',
                  WebkitBackdropFilter: 'blur(20px)',
                  boxShadow: '0 1px 0 rgba(255,255,255,0.08) inset, 0 0 0 1px rgba(255,255,255,0.02) inset, 0 20px 60px -20px rgba(0,0,0,0.6)',
                }}
                onMouseEnter={(e) => {
                ...
                onMouseLeave={(e) => {
                  ...
                  el.style.boxShadow = '0 1px 0 rgba(255,255,255,0.08) inset, 0 0 0 1px rgba(255,255,255,0.02) inset, 0 20px 60px -20px rgba(0,0,0,0.6)';
                  el.style.borderColor = 'rgba(255,255,255,0.06)';
                }}
              >
                <div className="absolute inset-x-6 top-0 h-px pointer-events-none" style={{ background: `linear-gradient(90deg, transparent, ${accent}80, transparent)`, opacity: 0.5 }} />
                <div className="absolute -top-20 -left-20 w-64 h-64 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" style={{ background: `radial-gradient(circle, ${accent}25 0%, transparent 60%)`, filter: 'blur(30px)' }} />
                <div className="absolute inset-x-0 -bottom-32 h-48 opacity-30 group-hover:opacity-60 transition-opacity duration-700 pointer-events-none" style={{ background: `radial-gradient(ellipse 60% 100% at 50% 100%, ${accent}50 0%, transparent 70%)`, filter: 'blur(20px)' }} />

                <div className="relative flex flex-col h-full min-h-[260px]">
                  <div className="flex items-start justify-between mb-6">
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-105" style={{ background: `linear-gradient(135deg, ${accent}25, ${accent}08)`, boxShadow: `0 0 0 1px ${accent}30 inset, 0 8px 20px -8px ${accent}40` }}>
                      <Icon className="w-5 h-5" style={{ color: accent }} />
                    </div>
                    <div className="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 group-hover:scale-110" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <div className="flex items-center justify-between pt-5 border-t border-white/[0.05] mt-auto">
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10.5px] font-medium tracking-wide" style={{ background: `${accent}12`, border: `1px solid ${accent}30`, color: accent }}>
                      <span className="relative flex w-1.5 h-1.5">
                        ...
                        <span className="relative w-1.5 h-1.5 rounded-full" style={{ background: accent, boxShadow: `0 0 8px ${accent}` }} />
                      </span>
                      <StatusIcon className="w-3 h-3" />
                      {card.statusLabel}
                    </div>
                    <span className="text-[11px] font-semibold uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-all duration-300 -translate-x-1 group-hover:translate-x-0" style={{ color: accent }}>
                      {card.cta} →
                    </span>
                  </div>
                </div>
